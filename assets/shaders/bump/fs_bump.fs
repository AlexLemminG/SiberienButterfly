...
vec3 CalcLightPBR(vec3 lightRadiance, vec3 lightDir, float extraCos, float extraCosReflection, Surface surface, vec3 viewDir){
	...
	float NDF = DistributionGGX(surface.normal, halfDir, extraCosReflection, 1.0);       
	float G   = GeometrySmith(surface.normal, viewDir, lightDir, extraCos, 1.0); 
...
void main()
{
	...
	//surface.emissive = vec3_splat(0.0);//emissive.rgb * u_emissiveColor;
	surface.normal = normalize(v_normal);
	surface.pos = v_wpos;
	surface.metalic = 0.0;//metalic;
	surface.roughness = 1.0;//roughness;
	surface.reflectionAtZeroIncidence = mix(vec3_splat(0.04), albedoAlpha.rgb, surface.metalic);
